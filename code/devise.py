from keras.layers import Lambda, Dense, BatchNormalization, Embedding, LSTM, concatenate, Input, Flatten, Dot
...
python devise.py -m VGG16 -e 1000 -v 30 -i n -db database_reduced.csv -d insert_comprehensive_description_of_all_param_changed -lr 0.001 -opt rmsprop -bs 128
...
parser.add_argument("--tate","-a",default="n")
parser.add_argument("--model","-o",default=None)
...
if __name__ == "__main__":
    ...
    (imfeatures, ccembeddings, emmat, labels, class_labels) = load_data(MAX_NUM_WORDS, WORD_DIM, network=args.imagemodel,tate=args.tate=="y", database = args.database)
    if args.image == "y":
        model = build_model(image_features, caption_features, emmat)
        plot_model(model, to_file='model.png')
    if args.tate == "y":
        #(imtrain, desctrain, labelstrain, imval, descval, labelsval) = zsl_split_data(ccembeddings, imfeatures, labels, class_labels, batch_size=BATCH, VALIDATION_SPLIT_PERCENTAGE=1)
    ...
    if args.savemodel == "y":
        model.save(args.description + ".h5")
